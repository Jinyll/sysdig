	...
	for(auto it = m_tinfo->m_cgroups.begin(); it != m_tinfo->m_cgroups.end(); ++it)
	{
		...
		if(pos != string::npos)
		{
				...
				cgroup.find_first_not_of("0123456789abcdefABCDEF", pos + 1) == string::npos)
			{
				...
				m_container_info.m_id = cgroup.substr(pos + 1, 12);
				return true;
			}
		}
		...
		if(pos != string::npos)
		{
			...
			{
				m_container_info.m_type = CT_DOCKER;
				m_container_info.m_id = cgroup.substr(pos + sizeof("docker-") - 1, 12);
				return true;
			}
		}
	}
...
void sinsp_container_engine_docker::fill_info()
{
	if(!m_query_os_for_missing_info)
	{
		return;
	}

	parse_docker(&m_container_info);
}

bool sinsp_container_engine_docker::parse_docker(sinsp_container_info *container)
{
	string json;
	sinsp_docker_response resp = get_docker(m_manager, "/v1.24", container->m_id, json);
	switch(resp) {
		case sinsp_docker_response::RESP_BAD_REQUEST:
			resp = get_docker(m_manager, "", container->m_id, json);
	...
	if(ip.empty())
	{
		...
		if(strncmp(net_mode.c_str(), "container:", strlen("container:")) == 0)
		{
			...
			if(!m_manager->get_container(pcnt.m_id, &pcnt))
			{
				parse_docker(&pcnt);
			}
...
bool sinsp_container_engine_lxc::resolve()
{
	for(auto it = m_tinfo->m_cgroups.begin(); it != m_tinfo->m_cgroups.end(); ++it)
	{
		string cgroup = it->second;
		...
		if(pos != string::npos)
		{
			...
			m_container_info.m_type = CT_LXC;
			m_container_info.m_id = cgroup.substr(id_start, id_end - id_start);
			return true;
		}
	}
	return false;
...
void sinsp_container_engine_lxc::fill_info()
{
	m_container_info.m_name = m_container_info.m_id;
}


bool sinsp_container_engine_libvirt_lxc::resolve()
{
	for(auto it = m_tinfo->m_cgroups.begin(); it != m_tinfo->m_cgroups.end(); ++it)
	{
		string cgroup = it->second;
			...
			pos == cgroup.length() - sizeof(".libvirt-lxc") + 1)
		{
			...
			if(pos2 != string::npos)
			{
				m_container_info.m_type = CT_LIBVIRT_LXC;
				m_container_info.m_id = cgroup.substr(pos2 + 1, pos - pos2 - 1);
				return true;
			}
		}
		...
		if(pos != string::npos)
		{
			...
			{
				m_container_info.m_type = CT_LIBVIRT_LXC;
				m_container_info.m_id = cgroup.substr(pos + sizeof("-lxc\\x2"), pos2 - pos - sizeof("-lxc\\x2"));
				return true;
			}
		}
		...
		{
			m_container_info.m_type = CT_LIBVIRT_LXC;
			m_container_info.m_id = cgroup.substr(pos + sizeof("/libvirt/lxc/") - 1);
			return true;
		}
...
void sinsp_container_engine_libvirt_lxc::fill_info()
{
	m_container_info.m_name = m_container_info.m_id;
}


bool sinsp_container_engine_mesos::resolve()
{
	for(auto it = m_tinfo->m_cgroups.begin(); it != m_tinfo->m_cgroups.end(); ++it)
	{
		string cgroup = it->second;
		...
		if(pos != string::npos)
		{
			...
			{
				m_container_info.m_type = CT_MESOS;
				m_container_info.m_id = move(id);
				...
				return set_mesos_task_id(&m_container_info, m_tinfo);
			}
		}
	}
	return false;
...
void sinsp_container_engine_mesos::fill_info()
{
	m_container_info.m_name = m_container_info.m_id;
}
...
bool sinsp_container_engine_rkt::resolve()
{
		...
		if(cgroup.rfind(SERVICE_SUFFIX) == cgroup.size() - SERVICE_SUFFIX.size())
		{
			...
			sinsp_threadinfo::visitor_func_t visitor = [&](sinsp_threadinfo* ptinfo)
			{
				for(const auto& env_var : ptinfo->get_env())
				{
					...
					{
						m_rkt_podid = env_var.substr(COREOS_PODID_VAR.size());
						return false;
					}
				}
				for(const auto& arg : ptinfo->m_args)
				{
					...
					{
						m_rkt_podid = arg.substr(SYSTEMD_UUID_ARG.size());
						return false;
					}
				}
				return true;
			...
			if(!m_rkt_podid.empty())
			{
				auto last_slash = cgroup.find_last_of("/");
				m_rkt_appname = cgroup.substr(last_slash + 1, cgroup.size() - last_slash - SERVICE_SUFFIX.size() - 1);

				char image_manifest_path[SCAP_MAX_PATH_SIZE];
				snprintf(image_manifest_path, sizeof(image_manifest_path), "%s/var/lib/rkt/pods/run/%s/appsinfo/%s/manifest", scap_get_host_root(), m_rkt_podid.c_str(), m_rkt_appname.c_str());
				...
				bool is_rkt_pod_id_valid = m_manager->container_exists(m_rkt_podid + ":" + m_rkt_appname); // if it's already on our table
#ifdef HAS_CAPTURE
				if(!is_rkt_pod_id_valid && m_query_os_for_missing_info)
				{
				...
				{
					m_container_info.m_type = CT_RKT;
					m_container_info.m_id = m_rkt_podid + ":" + m_rkt_appname;
					m_container_info.m_name = m_rkt_appname;
					return true;
				}
			}
		}
	...
	auto prefix = m_tinfo->m_root.find(COREOS_PREFIX);
	if(prefix == 0)
	{
		auto suffix = m_tinfo->m_root.find(COREOS_APP_SUFFIX, prefix);
		if(suffix != string::npos)
		{
			bool valid_id = false;
			m_rkt_appname = m_tinfo->m_root.substr(prefix + COREOS_PREFIX.size(), suffix - prefix - COREOS_PREFIX.size());
	...
	else
	{
		...
		auto prefix = m_tinfo->m_root.find(FLY_PREFIX);
		if(prefix == 0)
		{
			auto podid_suffix = m_tinfo->m_root.find(FLY_PODID_SUFFIX, prefix+FLY_PREFIX.size());
			if(podid_suffix != string::npos)
			{
				m_rkt_podid = m_tinfo->m_root.substr(prefix + FLY_PREFIX.size(), podid_suffix - prefix - FLY_PREFIX.size());
				auto appname_suffix = m_tinfo->m_root.find(FLY_APP_SUFFIX, podid_suffix+FLY_PODID_SUFFIX.size());
				if(appname_suffix != string::npos)
				{
					m_rkt_appname = m_tinfo->m_root.substr(podid_suffix + FLY_PODID_SUFFIX.size(),
...
void sinsp_container_engine_rkt::fill_info()
{
	if (!m_query_os_for_missing_info)
	{
		return;
